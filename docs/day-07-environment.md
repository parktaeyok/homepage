...
로컬 개발 서버는 실행하지 않는다. 코드 변경은 빌드로 확인하고, 화면과 사용자 흐름은 Vercel 연결 후 미리보기 배포에서 확인한다. 빌드 통과만으로 배포·상담 접수가 검증된 것은 아니다.
...
8~10일차에 4일차 문구와 5~6일차 시안을 실제 Next.js 홈페이지에 구현한다. 변경 후 빌드를 확인하고, Vercel 연결이 완료되면 미리보기 배포에서 화면과 문의 흐름을 확인한다.
